    ...
    <section id="observability" className="section-pad" style={{background:"#080c12",borderTop:"1px solid #1e2d40"}}>
      <div style={{maxWidth:"1200px",margin:"0 auto"}}>
        <div style={{marginBottom:"48px"}}>
          <div style={{fontFamily:"monospace",fontSize:"11px",color:"#06b6d4",letterSpacing:"4px",marginBottom:"12px"}}>// OBSERVABILITY_&_RELIABILITY</div>
        ...
        <div style={{padding:"24px",borderRadius:"10px",border:"1px solid #1e2d40",background:"rgba(6,182,212,0.03)",marginBottom:"28px"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"20px",flexWrap:"wrap",gap:"8px"}}>
            <div style={{fontFamily:"monospace",fontSize:"11px",color:"#06b6d4",letterSpacing:"2px"}}>SYSTEM DASHBOARD — LIVE METRICS</div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:"12px"}}>
            {metrics.map(m=>(
              <div key={m.label} style={{padding:"14px",borderRadius:"8px",border:"1px solid #1e2d40",background:"rgba(0,0,0,0.2)"}}>
                ...
                <div style={{fontSize:"10px",color:m.trend.startsWith("-")?m.label==="Error Rate"||m.label==="MTTR"?"#4ade80":"#f97316":"#60a5fa",fontFamily:"monospace"}}>{m.trend}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"20px"}}>
          {[
            {title:"Monitoring Stack",color:"#06b6d4",items:["AWS CloudWatch Metrics & Alarms","Container Insights for ECS","ALB access logs & request metrics","Custom business metrics","Multi-region health checks"]},
            {title:"Logging Architecture",color:"#8b5cf6",items:["Structured JSON logging in Lambda","CloudWatch Log Groups with retention","Log Insights for query & analysis","Error pattern alerting","Centralized log aggregation"]},
            {title:"Alerting & Incident Response",color:"#f97316",items:["SNS topic for critical alerts","PagerDuty-style escalation policies","Runbook documentation per alarm","Auto-remediation Lambda functions","Post-incident review process"]},
            {title:"Reliability Practices",color:"#22c55e",items:["SLO definition & error budget tracking","Blue/Green deployment strategy","Automated rollback on health failure","Circuit breaker patterns","Chaos engineering mindset"]},
          ].map(card=>(
            <div key={card.title} style={{padding:"20px",borderRadius:"10px",border:"1px solid #1e2d40",background:"rgba(255,255,255,0.01)"}}>
              ...
              {card.items.map(item=>(
                <div key={item} style={{display:"flex",gap:"10px",fontSize:"13px",color:"#8b949e",marginBottom:"8px",lineHeight:1.5}}>
                  <span style={{color:card.color,flexShrink:0,fontSize:"10px",marginTop:"3px"}}>></span>{item}
                </div>
              ))}
            </div>
          ))}
